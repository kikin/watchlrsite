...
class User(auth_models.User):
    ...
    def followers(self):
        follower_pk_list = UserFollowsUser.objects.filter(followee=self).values_list('follower', flat=True)
        follows = []
        for pk in follower_pk_list:
            follows.append(User.objects.get(pk=pk))
        return follows
